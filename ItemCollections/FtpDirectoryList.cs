...
namespace netCommander
{
    ...
    {
        private readonly string SORT_0_NAME = Options.GetLiteral(Options.LANG_FILE_NAME);
        private readonly string SORT_1_TIMESTAMP = Options.GetLiteral(Options.LANG_MODIFICATION_TIME);
        private readonly string SORT_2_SIZE = Options.GetLiteral(Options.LANG_FILE_SIZE);
        ...
        protected override void internal_sort(int criteria_index, bool reverse_order)
        {
            ...
            var new_list = new SortedList<FtpEntryInfo, object>(new_comparer);
        ...
        {
            var old_dir = internal_directory_path;
            var new_dir = string.Empty;
    ...
    public class FtpEntryInfo
    {
        ...
        {
            var try_count = FtpListParseFormats.Length;
            ...
            //now success
            //get groups
            var dir_value = reg_result.Groups["dir"].Value;
            ret.Directory = ((dir_value != "") && (dir_value != "-"));

            ret.Permission = reg_result.Groups["permission"].Value;

            var size_value = reg_result.Groups["size"].Value;
            var size_l = 0L;
            long.TryParse(size_value, out size_l);
            ret.Size = size_l;

            var ts_value = reg_result.Groups["timestamp"].Value;
            var ts_dt = new DateTime();
    ...
    public class FtpConnection
    {
        ...
        private void cache_update(string directory, List<FtpEntryInfo> contents)
        {
            lock (cache_lock)
            {
                var cache_list = new List<FtpEntryInfo>();
        ...
        {
            var directory = FtpPath.GetDirectory(path_name);
            var file = FtpPath.GetFile(path_name);
            var ret = false;
        ...
        public List<FtpEntryInfo> GetDirectoryDetailsList(string path_name, bool force_update)
        {
            ...
            if (force_update)
            {
                try
                {
                    ...
                    while ((line = s_reader.ReadLine()) != null)
                    {
                        var entry = FtpEntryInfo.FromDetailedListLine(line, path_name);
                        ret.Add(entry);
                    }
                    //update cache
        ...
        public FtpStatusCode DeleteFile(string path_name)
        {
            FtpWebRequest req = null;
            FtpWebResponse resp = null;
            var ret = FtpStatusCode.Undefined;
        ...
        public void CreateDirectoryTree(string path_name)
        {
            ...
            var current_info = new FtpEntryInfo();
            ...
            var parent_dir = FtpPath.GetDirectory(path_name);
            ...
            var parent_info = new FtpEntryInfo();
        ...
        private FtpWebRequest create_request(string path_name)
        {
            ...
            var ret = (FtpWebRequest)FtpWebRequest.Create(req_uri);
    ...
    public class FtpPath
    {
        ...
        public static string GetDirectory(string path)
        {
            ...
            //find last separator
            var last_separator_pos = path.LastIndexOf(PathSeparator);
        ...
        public static string GetFile(string path)
        {
            ...
            var splitted = path.Split(delim, StringSplitOptions.RemoveEmptyEntries);
        ...
        public static string Combine(string[] paths,int count)
        {
            ...
            var max = Math.Min(count, paths.Length);
